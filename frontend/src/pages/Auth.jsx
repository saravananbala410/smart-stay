  ...
  const F = ({ label, name, type = 'text', placeholder }) => (
    <div>
      ...
      <input type={type} required placeholder={placeholder}
        className="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        ...
        {sent ? (
          <div className="text-center py-8">
            ...
            <Link to="/login" className="text-indigo-600 mt-4 block hover:underline">Back to Login</Link>
          </div>
        ) : (
          <form onSubmit={submit} className="space-y-4 mt-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-indigo-600 text-white font-semibold py-3 rounded-lg disabled:opacity-60">
              {loading ? 'Sending...' : 'Send Reset Link'}
            </button>
            <Link to="/login" className="block text-center text-sm text-indigo-600 hover:underline">Back to Login</Link>
          </form>
        )}
      </div>
    </div>
